...
Func BrewSpells()
	...
	; Dark Spell Factory
	local $iBarrHere = 0
	local $TempLightningSpell = 0
	local $TempHealSpell = 0
	local $TempRageSpell = 0
	local $TempPoisonSpell = 0
	local $TempEarthSpell = 0
			...
			If isSpellFactory() Then
				If $LightningSpellComp > 0 Then ; Lightning Spells
                   $TempLightningSpell = Number(getBarracksTroopQuantity(175 + 107 * 0, 325))
				    ...
				    If $LightningSpellComp = 10 Then ; Auto Lightning Spells
						...
						ElseIf _ColorCheck(_GetPixelColor(200, 346 + $midOffsetY, True), Hex(0x414141, 6), 20) Then
							setlog("Spell Factory Full, Auto Train Lightning Spells is on", $COLOR_RED)
							Return
						 Else
								GemClick(334, 354 + $midOffsetY, $HealSpell, $iDelayTrain7, "#0290")
								SetLog("Auto Train Lightning Spells is on", $COLOR_BLUE)
			    				Return
							 EndIf
						  EndIf
				...
				If $HealSpellComp > 0 Then ; Heal Spells
                   $TempHealSpell = Number(getBarracksTroopQuantity(175 + 107 * 1, 325))
					...
					If _sleep($iDelayTrain2) Then Return
				    If $HealSpellComp = 10 Then ; Auto Heal Spells
						If _ColorCheck(_GetPixelColor(346, 375 + $midOffsetY, True), Hex(0xFFFFFF, 6), 20) = False Then
							setlog("Not enough Elixir to create Spell", $COLOR_RED)
							Return
						ElseIf _ColorCheck(_GetPixelColor(200, 346 + $midOffsetY, True), Hex(0x414141, 6), 20) Then
							setlog("Spell Factory Full, Auto Train Heal Spells is on", $COLOR_RED)
							Return
						Else
								GemClick(334, 354 + $midOffsetY, $HealSpell, $iDelayTrain7, "#0290")
								SetLog("Auto Train Heal Spells is on", $COLOR_BLUE)
								Return
							 EndIf
						  EndIf
				...
				If $RageSpellComp > 0 Then ; Rage Spells
					...
					If _sleep($iDelayTrain2) Then Return
				    If $RageSpellComp = 10 Then ; Auto Rage Spells
						If _ColorCheck(_GetPixelColor(453, 375 + $midOffsetY, True), Hex(0xFFFFFF, 6), 20) = False Then
							setlog("Not enough Elixir to create Spell", $COLOR_RED)
							Return
						ElseIf _ColorCheck(_GetPixelColor(200, 346 + $midOffsetY, True), Hex(0x414141, 6), 20) Then
							setlog("Spell Factory Full, Auto Train Rage Spells is on", $COLOR_RED)
							Return
						 Else
								GemClick(430, 354 + $midOffsetY, $RageSpell, $iDelayTrain7, "#0290")
								SetLog("Auto Train Rage Spells is on", $COLOR_BLUE)
								Return
							 EndIf
						  EndIf
