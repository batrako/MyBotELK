	...
	For $i = 1 To $max50
		For $t = 0 To $maxElixirLevel - 1
			If Int(Execute("$ElixirImages" & $t & "_50percent" & "[0]")) >= $i Then
				...
				setlog("Imagesearch return: ")
				setlog("- ElixirLocation : " & $ElixirLocation)
				setlog("- ElixirLocationx : " & $ElixirLocationx)
				setlog("- TElixirLocationy : " & $ElixirLocationy)
				ConsoleWrite(@CRLF)
				...
				If $ElixirLocation = 1 Then
					...
					If $debugBuildingPos = 1 Then
						ConsoleWrite("#*# ZombieSearch2: ", $COLOR_TEAL)
						ConsoleWrite("  - Position (" & $ElixirLocationx & "," & $ElixirLocationy & ")", $COLOR_TEAL)
						ConsoleWrite("  - Elixir Collector 50% level " & $t + 6, $COLOR_TEAL)
						ConsoleWrite("  - Image Match " & Execute("$ElixirImages" & $t & "_50percent" & "[" & $i & "]"), $COLOR_TEAL)
						ConsoleWrite("  - IsInsidediamond: " & isInsideDiamondXY($ElixirLocationx, $ElixirLocationy), $COLOR_TEAL)
						ConsoleWrite("  - Calculated  in: " & Round(TimerDiff($hTimer) / 1000, 2) & " seconds ", $COLOR_TEAL)
						ConsoleWrite("  - Images checked: " & $count, $COLOR_TEAL)
					EndIf
