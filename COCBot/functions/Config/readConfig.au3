...
Func readConfig() ;Reads config and sets it to the variables
	...
	If FileExists($config) Then
		...
		$PushToken2 = IniRead($config, "advanced", "AccountToken2", "") ; noyax telegram
...
;Noyax top Milking
		...
		$HysterGobs = IniRead($config, "attack", "HysterGobs", "40")
		$TempoTrain = IniRead($config, "attack", "TempoTrain", "15")
;Noyax bottom
		...
		$PushToken2 = IniRead($config, "pushbullet", "AccountToken2", "") ; noyax telegram
		...
		$pEnabled2 = IniRead($config, "pushbullet", "PBEnabled2", "0") ; noyax telegram
